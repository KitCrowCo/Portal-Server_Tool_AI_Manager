...
Pools are plain set-intersection/exclusion over CNode tags and ids, resolved fresh on every call - no caching,  o
historical optimization (that's the sDAG layer, deliberately deferre
). Selection within a pool is a single sta ic
score per priority mode using each CNode's own declared compute_score/quality_score - rough numbers today, expe ted
to be refined once real usage data (logged below) accumulates.
"""
import json, time
...
# --- Pool Resolution ---
# Pure successive set-filtering: Network (all CNodes) -> Pipeline pool (whitelist/blacklist tags+ids) -> Node's
# own cnode_tags (further intersected). Each level can only narrow, never widen, the level above it.
...
def pick_conn(candidates: list, conn_type: str, priority: str = "balanced") -> Optional[tuple]:
    """Returns (cnode, conn_dict_with_id) for the best-scoring candidate's first matching connection, or None if
    nothing qualifies. Callers should fail loudly on None - an empty pool after filtering is a configuration
...
def log_usage(cnode_id: str, node_type: str, elapsed_s: float, extra: dict = None):
    """Appends one line per node execution that went through pool resolution - passive dataset for a future sDAG
    cost model. Nothing reads this yet; safe to delete the file at any time."""
    try:
