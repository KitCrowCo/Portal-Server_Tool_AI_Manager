...
def register_step_type(name: str, fn, label: str = "", config_schema: dict = None): _STEP_TYPES[name] = {"fn": fn, "label": label or name, "config_schema": config_schema or {}} # label/config_schema are optional UI hints for Tessa's builder - the engine itself never reads them.
...
async def step_chat(config: dict, ctx) -> dict:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=10.0, read=600.0, write=10.0, pool=10.0)) as c:
        async with c.stream("POST", f"{_base(conn)}/api/chat", json=pl) as resp:
            if resp.status_code != 200: raise RuntimeError(f"chat step: HTTP {resp.status_code}")
            async for line in resp.aiter_lines():
                if not line: continue
                try: chunk = __import__("json").loads(line)
...
async def step_echo(config: dict, ctx) -> dict:
    ...
    Zero external dependencies - used for engine self-tests and as a manual inspection/breakpoint node when building real pipelines in Tessa."""
